import type { HistorialTrabajoReunionItem } from "@/modules/produccion/services/trabajosReunionServices";

const HIDDEN_FIELDS = new Set(["accion", "acciones"]);
const FIELD_LABELS: Record<string, string> = {
  titulo_trabajo: "Título del trabajo",
  nombre_reunion: "Nombre de la reunión",
  procedencia: "Procedencia",
  fecha_inicio: "Fecha de presentación",
  fecha_presentacion: "Fecha de presentación",
  tipo_reunion_id: "Tipo de reunión",
  grupo_utn_id: "UCT",
  enlace: "Enlace al trabajo o DOI",
};

export function formatTrabajoReunionContractValue(value: unknown) {
...
function getAutorEvent(item: HistorialTrabajoReunionItem) {
  ...
  return {
    ...
    name: formatTrabajoReunionContractValue(detalle),
    type: typeof detalle.tipo === "string" ? detalle.tipo.trim() : "",
  };
}

export function isVisibleTrabajoReunionHistoryItem(item: HistorialTrabajoReunionItem) {
...
function formatHistoryValue(value: unknown, field: string, tipoNames: Record<number, string>) {
  ...
  if (["fecha_inicio", "fecha_presentacion"].includes(field)) return formatDate(value);
  if (field === "tipo_reunion_id") {
    const name = tipoNames[Number(value)] ?? formatTrabajoReunionContractValue(value);
    return name || "Dato actualizado";
  }
  if (typeof value === "object") return formatTrabajoReunionContractValue(value) || "Dato actualizado";
  return String(value);
}

export function formatTrabajoReunionHistoryEntry(
  item: HistorialTrabajoReunionItem,
  tipoNames: Record<number, string> = {}
) {
  const authorPresentation = formatTrabajoReunionAuthorHistoryEntry(item);
  ...
  const title = FIELD_LABELS[normalizedField] ?? field.replace(/_id$/, "").replace(/_/g, " ").replace(/^./, (letter) => letter.toLocaleUpperCase("es"));
  return {
...
export function formatTrabajoReunionAuthorHistoryEntry(
  item: HistorialTrabajoReunionItem
) {
...
export function presentTrabajoReunionHistoryItems(items: HistorialTrabajoReunionItem[]) {
  return items.filter(isVisibleTrabajoReunionHistoryItem);
}
