...
rutas `POST`, `PUT`, `PATCH` y `DELETE` bajo `/api/v1/personal` y
`/api/v1/grupo`. Cada una debe responder `403` para un token con rol `LECTURA`.
La enumeracion evita que una nueva mutacion quede fuera de la prueba por olvido.
...
- Debe decidirse si GESTOR es global o queda limitado a una o varias UCT.
- Si se exige alcance, se necesita modelo de pertenencia usuario-UCT, comprobacion
  central reutilizable y pruebas cruzadas con recursos de dos UCT.
