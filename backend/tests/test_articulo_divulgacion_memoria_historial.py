...
class ArticuloDivulgacionMemoriaHistorialTestCase(unittest.TestCase):
    ...
    def setUp(self):
        self.add_patcher = patch("core.services.articulo_divulgacion_service.db.session.add")
    ...
    def test_snapshot_articulo_divulgacion_para_memoria_version_persiste_foto(self):
        ...
        articulo = SimpleNamespace(
            id=7,
            titulo="Divulgacion institucional",
            descripcion="Resumen de actividades del laboratorio",
            fecha_publicacion=date(2026, 4, 15),
            grupo_utn_id=4,
            grupo_utn=SimpleNamespace(nombre_sigla_grupo="GIDAS")
        )

        fake_query = SimpleNamespace(
            filter=lambda *args, **kwargs: SimpleNamespace(all=lambda: [articulo])
        )

        with patch(
            "core.services.articulo_divulgacion_service.ArticuloDivulgacion",
            new=SimpleNamespace(
                query=fake_query,
                deleted_at=SimpleNamespace(is_=lambda *_: None)
            )
        ):
            snapshots = ArticuloDivulgacionService.snapshot_para_memoria_version(
                version,
                user_id=27
            )

        self.assertEqual(len(snapshots), 1)
        self.assertEqual(snapshots[0].articulo_divulgacion_id, 7)
    ...
    def test_change_status_a_cerrada_genera_snapshot_articulos_divulgacion(self):
        ...
        mock_snapshot.assert_called_once_with(version, 101)
        self.assertEqual(resultado["version_actual"]["estado"], "cerrada")

    def test_obtener_historial_articulo_divulgacion_retorna_auditoria_ordenada(self):
        ...
        with patch(
            "core.services.articulo_divulgacion_service.db.session.get",
            ...
            historial = ArticuloDivulgacionService.get_historial(7)

        self.assertEqual(len(historial), 1)
        self.assertEqual(historial[0]["campo"], "titulo")
        self.assertEqual(historial[0]["usuario_nombre"], "admin")

    def test_obtener_snapshots_articulo_divulgacion_por_memoria_version(self):
        snapshot = SimpleNamespace(
            serialize=lambda: {
                "articulo_divulgacion_id": 7,
                "titulo": "Divulgacion institucional",
                "memoria_version_id": 111
            }
        )
        ...
        with patch(
            "core.services.articulo_divulgacion_service.ArticuloDivulgacionMemoriaVersion",
            new=SimpleNamespace(
                query=fake_query,
            ...
            resultado = ArticuloDivulgacionService.obtener_snapshots_por_memoria_version(111)

        self.assertEqual(len(resultado), 1)
        self.assertEqual(resultado[0]["articulo_divulgacion_id"], 7)
        self.assertEqual(resultado[0]["memoria_version_id"], 111)
